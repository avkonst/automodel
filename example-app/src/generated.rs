...
pub async fn test_json_query(
    client: &tokio_postgres::Client,
    test_data: serde_json::Value,
) -> Result<Option<crate::models::TestData>, tokio_postgres::Error> {
    let stmt = client.prepare("SELECT $1::jsonb as test_data").await?;
    let row = client.query_one(&stmt, &[&test_data]).await?;
    Ok(row
        .get::<_, Option<JsonWrapper<crate::models::TestData>>>(0)
...
pub async fn insert_user(
...
) -> Result<InsertUserResult, tokio_postgres::Error> {
    ...
    let row = client
        .query_one(&stmt, &[&name, &email, &age, &profile])
        .await?;
    Ok(InsertUserResult {
...
pub async fn get_all_users(
    client: &tokio_postgres::Client,
) -> Result<GetAllUsersResult, tokio_postgres::Error> {
    ...
    Ok(GetAllUsersResult {
        ...
        age: row.get::<_, Option<i32>>(3),
        profile: row
            .get::<_, Option<JsonWrapper<crate::models::UserProfile>>>(4)
            .map(|wrapper| wrapper.into_inner()),
...
pub async fn find_user_by_email(
    client: &tokio_postgres::Client,
    email: String,
) -> Result<FindUserByEmailResult, tokio_postgres::Error> {
...
pub async fn update_user_profile(
    ...
    profile: serde_json::Value,
    user_id: i32,
) -> Result<UpdateUserProfileResult, tokio_postgres::Error> {
