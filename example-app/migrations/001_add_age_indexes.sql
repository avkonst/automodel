...
-- SELECT id, name, email, age, profile, created_at FROM public.users WHERE age BETWEEN ${min_age} AND ${max_age} AND updated_at > NOW() - INTERVAL '30 days'
...
CREATE INDEX IF NOT EXISTS idx_users_age_updated_at ON public.users(age, updated_at);
...
-- CREATE INDEX IF NOT EXISTS idx_users_updated_at_age ON public.users(updated_at, age);
-- Add some statistics public.comments
